...
extension AppDelegate {
    func setUpAppearance() {
        ...
        UINavigationBar.appearance().titleTextAttributes = [.foregroundColor: UIColor(named: "TMGrey")!, .font: UIFont.taskManagerFont(ofSize: 17, weight: .bold)]
        ...
        for state: UIControlState in [.normal, .disabled, .highlighted, .selected] {
            UIBarButtonItem.appearance().setTitleTextAttributes([.font: UIFont.taskManagerFont(ofSize: 17, weight: .regular), .foregroundColor: UIColor(named: "TMGrey")!], for: state)
        }
    }
}
